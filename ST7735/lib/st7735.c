...
const uint8_t INIT_ST7735B[] PROGMEM = {
  ...
  // 0xA0 = 1010 0000
  1,   0, MADCTL, 0xA0,
  //2, 0, DISSET5, 0x15, 0x02,
  //1, 0, INVCTR, 0x0,
  //2, 10, PWCTR1, 0x02, 0x70,
  //1, 0, PWCTR2, 0x05,
  //2 , 0, PWCTR3, 0x01,0x02,
  //4, 0, CASET, 0x00, 0x02, 0x00, 0x81,
  //0,10, NORON,
  // Main screen turn on
  0, 200, DISPON 
  // ---------------------------------------
  ...
  if (size == X1) {  
    ...
    while (idxCol--) {
      ...
      while (idxRow--) {
        ...
        ST7735_DrawPixel (lcd, cacheMemIndexCol + idxCol, cacheMemIndexRow + idxRow, BLACK);
        }
      }
    ...
    while (idxCol--) {
      ...
      while (idxRow--) {
        ...
        if (letter & (1 << idxRow)) {
          ...
          // draw first left up pixel; 
          // (idxRow << 1) - 2x multiplied 
          ST7735_DrawPixel (lcd, cacheMemIndexCol + idxCol, cacheMemIndexRow + (idxRow << 1), BLACK);
          // draw second left down pixel
          ST7735_DrawPixel (lcd, cacheMemIndexCol + idxCol, cacheMemIndexRow + (idxRow << 1) + 1, BLACK);
        }
      }
      // fill index row again
      idxRow = CHARS_ROWS_LEN;
    ...
    while (idxCol--) {
      ...
      while (idxRow--) {
        ...
        if (letter & (1 << idxRow)) {
          ...
          // draw first left up pixel; 
          // (idxRow << 1) - 2x multiplied 
          ST7735_DrawPixel (lcd, cacheMemIndexCol + (idxCol << 1), cacheMemIndexRow + (idxRow << 1), BLACK);
          // draw second left down pixel
          ST7735_DrawPixel (lcd, cacheMemIndexCol + (idxCol << 1), cacheMemIndexRow + (idxRow << 1) + 1, BLACK);
          // draw third right up pixel
          ST7735_DrawPixel (lcd, cacheMemIndexCol + (idxCol << 1) + 1, cacheMemIndexRow + (idxRow << 1), BLACK);
          // draw fourth right down pixel
          ST7735_DrawPixel (lcd, cacheMemIndexCol + (idxCol << 1) + 1, cacheMemIndexRow + (idxRow << 1) + 1, BLACK);
        }
      }
      // fill index row again
      idxRow = CHARS_ROWS_LEN;
